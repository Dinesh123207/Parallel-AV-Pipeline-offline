...
// // This would typically come from an API, but for demonstration purposes, we'll define it here
// const dummyFolders = [
//   ".",
//   "A",
//   "B",
...
interface NwFolderProps {
  ...
  setSelectedPc: React.Dispatch<
    React.SetStateAction<PcDetails>
  >;
}

export const NwFolder: React.FC<NwFolderProps> = ({
  selectedPc,
  setSelectedPc,
}) => {
  ...
  const isOpen = selectedPc.name !== "" && selectedPc.ip !== "";
  ...
  const getCurrentFolders = async (): Promise<void> => {
    try {
      ...
      setFolders(response); // Store the fetched folders in the state
  ...
  useEffect(() => {
    if (selectedPc.name !== "") getCurrentFolders();
  }, [currentPath, selectedPc]);
  ...
  }, [currentPath, selectedFolder, selectedPc]);
        ...
        <div className="flex flex-col flex-grow overflow-hidden pb-1">
              ...
              {!isFoldersLoading &&
                folders.map((folder) => (
                  <button
                    ...
                    className={`flex flex-col items-center p-2 rounded w-full ${selectedFolder === folder
                      ? "bg-blue-100"
                      : "hover:bg-gray-100"
                      }`}
          ...
          <div className="sticky bottom-0 right-0 px-2 py-2 bg-secondary_bg-1 flex justify-end">
            ...
            >
              ...
              {isLoading ? <LoadingSpinner size={1} /> : <> <Check className="mr-2 h-4 w-4" />  "Select" </>}
            </Button>
          </div>
        </div>
      </DialogContent>
